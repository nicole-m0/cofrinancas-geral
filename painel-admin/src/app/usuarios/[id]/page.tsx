...
import { categoryDistribution, goalProgress } from '@/lib/aggregate';
import { brl, brlShort, pct, shortDate } from '@/lib/format';
import { getUser, userGoals, userRecurring, userTransactions, users } from '@/lib/mock';
...
export default async function UsuarioDetalhePage({ params }: PageProps<'/usuarios/[id]'>) {
  ...
  const goals = userGoals(id);
  const recs = userRecurring(id);
  const dist = categoryDistribution(txs);
  ...
  return (
    <>
      <PageHeader
        ...
        actions={
          <>
            <Badge tone={user.status === 'ativo' ? 'green' : 'neutral'}>
      ...
      <div className="mb-4 flex items-center gap-4 rounded-2xl border border-hair bg-card p-5">
        ...
        <div>
          ...
          <div className="text-xs font-medium text-muted">
            Entrou em {shortDate(user.joinedAt)} · {goals.length} metas · {txs.length} transações
          </div>
        </div>
      </div>
      ...
      <section className="mt-4 grid gap-4 lg:grid-cols-2">
        <Card>
          <h2 className="mb-3 text-sm font-extrabold text-ink">Metas ({goals.length})</h2>
          <div className="flex flex-col gap-4">
            ...
            {goals.map((g) => (
              <div key={g.id}>
                ...
                <ProgressBar value={goalProgress(g)} color={g.color} />
        ...
        <Card>
          <h2 className="mb-3 text-sm font-extrabold text-ink">Recorrentes ({recs.length})</h2>
            ...
            {recs.map((r) => (
              <li
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <div className="text-xs font-medium text-muted">
                    {r.frequency} · próx. {shortDate(r.nextDate)}
                  </div>
                </div>
